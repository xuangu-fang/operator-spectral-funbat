...
### 3.5 Predictive uncertainty（5 seeds，两边 global routing）

来源 `uq_summary.json`。目标是**独立重新加噪的 held-out 值**，所以量的是对观测的校准。

| 观测率 | arm | NRMSE | 95% 覆盖率 | 预测 NLL | 区间宽度 |
|---|---|---:|---:|---:|---:|
| 1% | **operator** | 0.4347 | **0.837** | **+0.725** | 1.142 |
| 1% | generic | 0.4530 | 0.775 | +1.018 | 1.004 |
| 2% | **operator** | 0.3568 | **0.861** | **+0.457** | 1.020 |
| 2% | generic | 0.3625 | 0.850 | +0.492 | 1.004 |
| 5% | operator | 0.2946 | 0.912 | +0.204 | 1.035 |
| 5% | generic | 0.2967 | 0.910 | +0.211 | 1.035 |

**正面**：先验在校准上也一致更好，且优势**同样集中在最稀疏处**——1% 时覆盖率高 6.2 个百分点、NLL 低 0.29；5% 时两者基本相同。这与点预测 margin 的收缩是同一模式，互为佐证。

**必须承认的负面（写进 limitation，不回避）**：

1. **两者在 1–2% 都严重欠覆盖**（0.837 / 0.775，名义 0.95）。这是**宿主模型的校准问题，不是先验能修的**。
2. operator 的区间更宽（1.142 vs 1.004），因此它的覆盖优势**部分来自更诚实地承认不确定性**，而非纯粹更准。
3. 这是**有限特征 + mean-field 变分**的 posterior predictive，不是解析 exact GP posterior；NLL 还带 64-sample 的 MC 误差。

> **本文的主张是"更好的点预测先验"，不是"更好的不确定性量化"。** UQ 表作为附加证据报告，不作为卖点。
